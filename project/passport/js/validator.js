...
return false;
case "AjaxValidator":
if (stype == "text" || stype == "textarea" || stype == "file" || stype == "password" || stype == "select-one" || stype == "tel" || stype == "number") {
return true
} else {
return false
}
...
tip.html(showmsg)
if(showclass == 'fl onError'){
	$('#'+setting0.tipid+'Div').removeClass('okle');
	$('#'+setting0.tipid+'Div').addClass('errorle');
}else if(showclass == 'fl onCorrect'){
	$('#'+setting0.tipid+'Div').removeClass('errorle');
	$('#'+setting0.tipid+'Div').addClass('okle');
}else{
	$('#'+setting0.tipid+'Div').removeClass('okle');
	$('#'+setting0.tipid+'Div').removeClass('errorle');
}
...
$.formValidator.setTipState(this, "fl onShow", this.settings[0].onshow)
})
},
setFailState: function(tipid, showmsg) {
var tip = $("#" + tipid);
tip.removeClass();
tip.addClass("fl onError");
tip.html(showmsg)
$('#'+tipid+'Div').removeClass('errorle');
$('#'+tipid+'Div').addClass('okle');
},
...
$.formValidator.setTipState(elem, "fl onCorrect", showmsg)
}
return showmsg
},
...
if( initConfig.alertmessage && thefirsterrmsg) alert(thefirsterrmsg);
...
var rt = setting.success(data);
if ( rt === true ) {
$.formValidator.setTipState(elem, "fl onCorrect", settings[0].oncorrect);
setting.isvalid = true
} else if(typeof rt == 'string'){
$.formValidator.setTipState(elem, "fl onError", rt );
setting.isvalid = false
} else {
	
$.formValidator.setTipState(elem, "fl onError", setting.onerror);
setting.isvalid = false
}
},
complete: function() {
...
if (!initConfig.alertmessage) {
$.formValidator.setTipState(this, "fl onShow", setting.onshow)
}
...
$.formValidator.setTipState(this, "fl onFocus", setting.onfocus)
}
});
...
var ls_style = onshow ? "fl onShow" : "fl onCorrect";
alert(2);
